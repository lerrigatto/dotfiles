...
minSecsLeft () { echo $(( $(secondsLeft) % 60 )) ; }
minutesLeft () { echo $(( $(secondsLeft) / 60 )) ; }

printExpiryTime () { notify-send -u low "Timer expires at $( date -d "$(secondsLeft) sec" +%H:%M)" ;}

deleteExpiryTime () { notify-send "Timer cancelled" ; }
...
updateTail () {
  ...
  if timerRunning
  then
    echo "$(timerLabel) $(minutesLeft):$(minSecsLeft)"
  else
    echo "${STANDBY_LABEL}"
  fi
}
